...
logger = logging.getLogger("health-runner")
...
app = FastAPI(title="Ecommerce Health Runner", docs_url=None, redoc_url=None, openapi_url=None)
...
async def health_check(request: Request, payload: HealthPayload):
    ...
    if not check_ids:
        logger.info("report_id=%s skipped: no checks configured", report_id)
    ...
    try:
        ...
        logger.warning("report_id=%s rejected: runner busy", report_id)
        raise HTTPException(status_code=429, detail="Runner is busy") from error
    started = time.monotonic()
    logger.info("report_id=%s starting %d check(s)", report_id, len(check_ids))
    try:
        checks = []
        ...
        logger.info("report_id=%s finished status=%s summary=%s duration_ms=%d", report_id, status, summary, int((time.monotonic() - started) * 1000))
        return {
            "compact_version": "v1",
            "status": status,
            "skipped": False,
